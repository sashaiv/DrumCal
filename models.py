"""
Модели данных для Length Helper.
Все dataclasses — чистые данные без бизнес-логики.
"""
...
class ProcessParams:
    ...
    # Добавляется к длине прогона изолирования: нужно изолировать
    # (total_batch + n_segments × tolerance) метров ТПЖ.

    keep_journal_order: bool = False
    # П5: True → при упаковке партий скрутки НЕ сортировать сегменты по убыванию,
    # а брать в порядке кабельного журнала.

    waste_warning_threshold_m: float = 50.0
    # П6: остаток катушки/барабана жилы после использования < порога →
    # предупреждение «вероятно в отход».
...
class InsulationRun:
    ...
    # П1+П9: сколько реально снимается с барабана ТПЖ = length + startup_loss.
