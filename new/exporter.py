"""
Экспорт результатов планирования в Excel (v5).

Улучшения относительно root/exporter.py:
  - Отображение group_label ('5ж', '4ж-А', …) в заголовке партии скрутки
  - Поддержка flexible-кабелей в сводке
"""
...
def _sheet_twisting(wb: openpyxl.Workbook, result: PlanResult, pf_map: dict):
    ...
    for batch in result.batches:
        ...
        # ── Заголовок партии ─────────────────────────────────────────────
        group_part = f'   |   Группа: {batch.group_label}' if batch.group_label else ''
        batch_title = (
